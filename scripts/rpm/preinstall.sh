...
then
  /bin/echo "Tyrant is not running"
else
  sleepCounter=0
  sleepIncrement=2
  waitTimeOut=600
  ...
  /bin/echo "Tyrant is running, stopping service"
...
then
  mkdir -p $prefixDir
  /usr/sbin/useradd -r -s /sbin/nologin -d $prefixDir -m -c "Tyrant user for the Tyrant service" $APP_NAME 2> /dev/null || :
fi
