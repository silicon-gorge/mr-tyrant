...
then
  /bin/echo "Tyranitar is not running"
else
  sleepCounter=0
  sleepIncrement=2
  waitTimeOut=600
  ...
  /bin/echo "Tyranitar is running, stopping service"
...
then
  mkdir -p $prefixDir
  /usr/sbin/useradd -r -s /sbin/nologin -d $prefixDir -m -c "Tyranitar user for the Tyranitar service" $APP_NAME 2> /dev/null || :
fi
