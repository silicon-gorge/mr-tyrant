...
(fact "that failing to get the environment list gives back nil"
      (environments) => nil
      (provided
       (http/get "http://onix/1.x/environments" anything) => {:status 404}))

(fact "that our environment list comes back properly"
      (environments) => ..environments..
      (provided
       (http/get "http://onix/1.x/environments" anything) => {:body ..body..
                                                                   :status 200}
...
(fact "that failing to get an environment gives back nil"
      (environment "env") => nil
      (provided
       (http/get "http://onix/1.x/environments/env" anything) => {:status 404}))

(fact "that we can retrieve an environment"
      (environment "env") => ..environment..
      (provided
       (http/get "http://onix/1.x/environments/env" anything) => {:body ..body..
                                                                       :status 200}
